...
- **File or Directory Targets**: Snapshot whole directories, individual files, or a mix of both.
...
- **AI Context Summary**: Estimate emitted bytes, lines, and rough token counts before sending context to a model.
...
```bash
git clone https://github.com/gustavgauge/treecat.git
cd treecat
mkdir -p ~/.local/bin
ln -sf "$PWD/treecat.sh" ~/.local/bin/treecat
```

## Usage

Once installed, you can run `treecat` from any directory. Paths can be directories or individual files.

```bash
treecat [OPTIONS] [--] [PATH1 [PATH2 ...]]
```
...
Snapshot one file without including the rest of its directory.
```bash
treecat README.md
treecat -t src/index.js
```
...
Preview file sizes and rough token counts without printing the full snapshot.
```bash
treecat -b --summary
treecat -b --tokens --max-lines 200
```
...
ShellCheck is the only development dependency. Run the lightweight shell test suite and ShellCheck before changing behavior:

```bash
tests/run.sh
shellcheck treecat.sh tests/run.sh
```
...
```
### Directory structure (generated by treecat on 2026-05-22 17:14:46)
my-project
├── README.md
└── src
    ├── index.js
...
-------- ---------- ---------- ---------- ----------  ----
text     included         245         12         62  README.md
text     included          94          4         24  src/index.js

Files included: 2
Text files: 2
Binary files included: 0
Binary files skipped: 0
Empty/missing files skipped: 0
Estimated emitted bytes: 339
Estimated emitted lines: 16
Estimated tokens: 86

Token estimate uses a rough 4 bytes/token heuristic.
```
